...
namespace Violet {
	...
	void ScriptEngine::LoadAssemblyClasses()
	{
		...
		for (int32_t i = 0; i < numTypes; i++)
		{
			...
			const char* name = mono_metadata_string_heap(s_MonoData->AppAssemblyImage, cols[MONO_TYPEDEF_NAME]);
			std::string fullName;
			...
			MonoClass* monoClass = mono_class_from_name(s_MonoData->AppAssemblyImage, nameSpace, name);
